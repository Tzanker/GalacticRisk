...
import { getNetworkNodeCharacteristics, changeSystemColor } from  './Utils.js'
let network = null;
let nodes = [];
let edges = [];
...
$('main').prepend('<input type="button" class="btn btn-dark" id="saveButton" value="Save">')
document.getElementById('saveButton').addEventListener('click', ()=>{
    saveMap({paths:network.body.data.edges._data,nodes: getNetworkNodeCharacteristics(network)});
});
...
async function assembleMap() {
    ...
    for (index in map.nodes) {
        ...
        let divisionNumbered=''
        for(let divIndex in system.divisions){
            divisionNumbered += system.divisions[divIndex].id+ '<br>'
        }
        title='' + system.name  + '<hr class="tooltip-hr"> divisions:<br>' +divisionNumbered;
